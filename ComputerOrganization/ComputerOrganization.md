...
补码由低位到高位找第一个 1，其左边与与反码相同，其右边与原码相同
...
> 通常情况下，L1 Cache会被划分成指令Cache和数据Cache，分别用于存储指令和数据，而L2 Cache、L3 Cahce等则是指令和数据混合存储
...
1. 都是为了提高性能，有容量速度价格的梯度

2. 都把数据划分为小数据块，并作为基本传递单位

    虚存系统的信息块更大

3. 都有地址的映射、替换算法、更新策略等问题

4. 依据程序的局部性原理应用“快速缓存”的思想，把活跃的数据放在相对高速的部件中
...
2.某机器有一个标志寄存器，其中有进位/借位标志 CF、零标志 ZF、符号标志 SF 和溢出标志 OF，条件转移指令 bgt（无符号整数比较大于时转移）的转移条件是（）
...
3.按字节编址的计算机中，某 double 型数组 A 的首地址为 2000H，使用变址寻址和循环 结构访问数组 A，保存数组下标的变址寄存器初值为 0，每次循环取一个数组元素，其偏移地 址为变址值乘以 sizeof(double)，取完后变址寄存器内容自动加 1。若某次循环所取元素的地址 为 2100H，则进入该次循环时变址寄存器的内容是（）
...
调用 `ret` 指令，会取 esp 指向的指，赋给 eip，使得指令指向 P 中的 `call` 指令的下一条指令
...
2. 地址总线 AB：传输地址信息，包括主存单元或 I/O 端口的地址；位数（根数与主存地址大小及设备数量有关）；单向

3. 控制总线 CB：传输控制信息；一根控制线传输一个信号；有出：CPU 送出的控制命令；有入：主存（或外设）返回 CPU 的反馈信号
...
<table>
	...
	<tr>
		...
		<td align="center" valign=middle>为配合 32 位 CP U而设计的扩展 ISA 总线</td>
...
&nbsp;|中断|DMA
:----:|:--:|:-:
数据传送|软件控制，程序的切换 -> 保存和恢复现场|硬件控制，CPU 只需进行预处理和后处理|中断请求|传送数据|后处理
请求资源|CPU 处理时间|总线使用权
