- [计算机网络](#计算机网络)
  ...
  - [第四章 网络层](#第四章-网络层)
    ...
    - [4.7 移动 IP](#47-移动-ip)
      ...
      - [4.7.1 移动 IP 的概念](#471-移动-ip-的概念)

      - [4.7.2 移动 IP 通信的过程](#472-移动-ip-通信的过程)

    - [4.8 互联网连接问题](#48-互联网连接问题)

    - [4.9 网络层设备](#49-网络层设备)

      - [4.9.1 冲突域与广播域](#491-冲突域与广播域)

      - [4.9.2 路由器的组成和功能](#492-路由器的组成和功能)

      - [4.9.3 路由表与路由转发](#493-路由表与路由转发)
...
2.将 1 路模拟信号分别编码为数字信号后，和另外 7 路数字信号采用同步时分复用（同步 TDM）复用一条信道。1 路模拟信号的频率变化范围是 0~1 kHz，每个采样点用的编码方式是 4 位二进制数，另外 7 路数字信号的数据率均为 7.2 kb/s，复用线路中最小的通信能力是（）
...
3.在下图所示的采用“存储—转发”方式的分组交换网络中，所有链路的数据传输速率为 100 Mbit/s，分组大小为 1000B，其中分组头大小为 20 B。若主机 H1 向主机 H2 发送一个大小为 980000 B 的文件，则在不考虑分组拆装时间和传播延迟的情况下，从 H1 发送开始到 H2 接收完为止，需要的时间至少是（）
...
1. 确定校验码位数 r
    ...
    为了使校验结果可以指出海明码任一位出错的位置，则 r 需要满足海明不等式 $m + k <= 2^k - 1$
...
由于无线信道的通信质量远不如有线信道，802.11 使用链路层确认/重传（ARQ）方案
...
总长度 Total Length|首部 + 数据的长度，单位为 1 字节，最大为 $2^{16}-1=65535$ B，但是实际上永远不会达到该长度，因为有 MTU 的限制|16 位
