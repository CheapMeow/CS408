...
打开定时器属于时钟管理的内容，对时钟的操作必须加以保护，否则，一个用户进程可以在时间片还未到之前把时钟改回去，从而导致时间片永远不会用完，那么该用户进程就可以一直占用CPU，这显然不合理
...
> 可以说“内部异常的产生与当前执行指令相关”，虽然我看这个“电源掉电”怎么看也不像是与指令有关……

> 可以说“内部异常处理后不一定能返回到发生异常的指令继续执行”，虽然像是“缺页”这种故障是可以回到原位置的，但是对于“除数为 0”这种故障是不能回到原位置的
...
<img src="./virtual-machine.svg" height = "250"/>
...
3. 阻塞态（等待态）
    ...
    进程需要其他资源（除了处理器）或等待某一事件，例如 P 操作
...
2. 高级通信
    ...
    3. 管道通信
        ...
        管道实际上是一种固定大小的缓冲区，管道对于管道两端的进程而言，就是一个文件，但它不是普通的文件，他不属于某种文件系统，而是自立门户，单独构成一种文件系统，并且只存在于内存中

        半双工，不能两个方向同时进行

        管道的容量大小通常为内存上的一夜，它的大小与磁盘容量大小无关

        当管道满时，写进程阻塞；当管道空时，读进程阻塞
...
> 有些操作系统的调度单位就是线程，所以每个线程，不管是用户还是内核线程，都有自己的 TCB
>
...
注意阻塞进程的资源得到满足后，不是直接进入运行，而是先转变为就绪态
...
不妨如上图所示，将十字路口车道的公共区域分为 4 块，分别为图上的 1, 2, 3, 4
...
:----:|:----------:|:----------:|:-------:|:-----:|:-----:|
死锁预防|保守，宁可资源闲置|一次请求所有资源，资源剥夺，资源按序分配|适用于突发式处理的进程，不必进行剥夺|效率低，进程初始化时间延长；剥夺次数过多；不便灵活申请新资源|资源有序分配
死锁避兔|是“预防”和“检测”的折中（在运行时判断是否可能死锁）|寻找可能的安全允许顺序|不必进行剥夺|必须知道将来的资源需求；进程不能被长时间阻塞|银行家算法
死锁检测|宽松，只要允许就分配资源|定期检查死锁是否已经发生|不延长进程初始化时间，允许对死锁进行现场处理|通过剥夺解除死锁，造成损失|资源分配图、撤销进程
...
m 个进程，每个进程都至多需要 n 个资源

能够保证出现死锁的最大资源数是 m\*(n-1)

刚好再加 1 个资源就可以让其中 1 个进程完成

则该系统必然不会发生死锁的最少资源是 m\*(n-1) + 1

> 我之前做题做错了就是在想那他进程赖着不走怎么办
>
> 这个时候认为一定会有进程结束，而不是认为“所有进程都不再申请新资源而导致死锁”

##### 例题：死锁的处理策略辨析

1.若系统 S1 采用死锁避免方法，S2 采用死锁检测方法。下列叙述中，正确的是 （）。
...
银行家算法是最著名的**死锁避免**算法，其中的最大需求矩阵 MAX 定义了每一个进程对 m 类资源的最大需求量，系统在执行安全性算法中都会检查此次资源试分配后，系统是否处于安全状态，若不安全则将本次的试探分配作废

在死锁的检测和解除中，在系统为进程分配资源时**不采取任何措施**，但提供死锁的检测和解除的手段，故 Ⅱ、Ⅲ 正确
...
3. 装入（装载）
    ...
    3. 动态运行时装入 动态重定位
        ...
        可以把程序放在不连续的存储区中，在程序运行之前装入部分代码就可以运行
        
        然后在程序运行的时候根据需要动态申请内存，执行到访存指令时，通过硬件地址变换机构将逻辑地址转换为主存中的物理地址

> 以 C 语言为例：C 语言经过预处理(cpp) → 编译(ccl) → 汇编(as) → 链接(1d) → 可执行文件
> 
> 其中链接的前一步，产生了可重定位的二进制的目标文件
> 
> C 语言采用源文件独立编译的方法，如程序 main.c，file1.c，file2.c，file1.h，file2.h，在链接的前一步生成了 main.o，file1.o，file2.o
> 
> 链接器将这三个文件，libc 和其他的库文件链接成一个可执行文件
> 
> 链接阶段主要完成了重定位，形成逻辑的地址空间
>
...
<table>
    <tr>
        <td align = center>内存的管理方式</td>
        <td align = center>内存的分配方式</td>
        <td align = center>重定位方式</td>
    </tr>
    <tr>
        <td align = center rowspan = 3>连续分配</td>
        <td align = center>单一连续分配</td>
    </tr>
    <tr>
        <td align = center>固定分区分配</td>
    </tr>
    <tr>
        <td align = center>动态分区分配</td>
    </tr>
    <tr>
        <td align = center rowspan = 3>非连续分配</td>
        <td align = center>分段式分配</td>
        <td align = center rowspan = 3>都有地址变换机构</br>动态重定位</td>
    </tr>
    <tr>
        <td align = center>分页式分配</td>
    </tr>
    <tr>
        <td align = center>段页式分配</td>
    </tr>
</table>

##### 例题：多进程的内存保护

1.多进程在主存中彼此互不干扰的环境下运行，操作系统是通过（）实现的
...
2. 虚存 <= 计算机位数所能容纳的最大容量
    ...
    即硬件的访存能力，一般由地址总线宽度决定

##### 例题：存储覆盖技术不叫作虚拟存储技术的存储管理方式
...
> 然而注意，访问逻辑地址 a 时若产生缺页中断，中断处理之后还是要再次访问 TLB（如有）、Page 才能得到 a 的物理地址
>
> 而不是有一种错觉好像中断处理之后我就直接得到了 a 的物理地址，不是的
...
1.若用户进程访问内存时产生缺页，则下列选项中，操作系统可能执行的操作是（）
...
一方面，只有已经加载到内存的数据或单元才可能越界现象，而缺页中断时还没有加载到内存

另一方面，缺页中断之后置换页或者分配内存（调入页框），再次访问上一次要访问的内存时，就一定能访问到

因此不会有越界错

##### 例题：注意页表寻找的是整个页号空间而不是虚拟地址空间

2.一个 64 位的计算机系统中，地址线宽为 64 位，实际使用的虚拟地址空间的大小是 2^48，若采用虚拟页式存储管理，每页的大小为 2^13，即 8 KB，页表表项长为 8 字节，采用多级页表进行管理，那么，多级页表的级次最小是（）
...
其实很简单嘛，一个页全放页表项的话，可以放 2^13 / 8 = 2^10 个页表项

我一开始看“虚拟地址空间的大小是 2^48”，我就直接 48 / 10 = 4.8 所以选了五级

但是页表寻找的是整个页号空间而不是虚拟地址空间

所以虚拟地址空间占 2^48 / 2^13 = 2^35 个页，所以应该是 35 / 10 = 3.5 所以选四级，B
...
##### 例题：一般认为访问存储方式正确的数组不会导致很高的缺页率

1.总体上说，“按需调页”是一个很好的虚拟内存管理策略。但是，有些程序设计技术并不适合于这种环境，例如（）
...
C 也有空间局部性，因为访问了 A[i] 之后紧接着访问 A[i+1]

我做错的原因是想到有一个 Cache 的题，它展示了当同时访问两个数组时，两个数组可能互相争抢同一个 Cache 行，导致 Cache 命中率下降

但是这个题嘛……讲的是页，应该，其实，大概，很难发生这种小数据范围上的争抢吧

或许那个题只是把页的大小故意写小了，方便做题

D 二分是跳跃的，空间局部性很差，故选 D
...
2. 改进型 CLOCK
    ...
    1. 找第一个 A = 0, M = 0，期间不改变访问位 A。若没找到，进入 2
...
##### 例题：虚拟存储器没有要求程序装入比例的说法

2.下面关于虚拟存储器的论述中，正确的是（）
...
页面的大小是由操作系统决定的，不同的操作系统的分页机制可能不同，**对用户透明**，B 错误

虚拟存储器只装入部分作业到内存是为了从逻辑上扩充内存，有些较小的程序一页便可全部装入，但没有“10%~30%”的说法，C 错误

最佳适应算法是内存的**动态分区分配**中的算法，D 错误

> 动态运行时装入（动态重定位）中也有部分装入的说法
>
> 与虚拟存储器无关……就是联想一下
...
<img src = "./working-set.png" height = 100>
...
1.某文件系统中，针对每个文件，用户类别分为 4 类：安全管理员、文件主、文件主的伙伴、其他用户；访问权限分为 5 种：完全控制、执行、修改、读取、写入。若文件控制块中用二进制位串表示文件权限，为表示不同类别用户对一个文件的访问权限，则描述文件权限的位数至少为（）
...
I. 索引顺序文件也是一种特殊的顺序文件，因此通常存放在磁带上

Ⅱ. 索引顺序文件既能顺序访问，又能随机访问

Ⅲ. 存储在直接存取存储器上面的文件也能顺序访问，但一般效率较差

IV. 在磁带上的顺序文件中添加新记录时，必须复制整个文件
...
对于 I，直接存取存储器（磁盘）既不像 RAM 那样随机地访问任一个存储单元，也不像顺序存取存储器(如磁带)那样完全顺序存取，而是介于两者之间，存取信息时通常先寻找整个存储器的某个小区域(如磁盘上的磁道)再在小区域顺序查找。所以直接存取不完全等于随机存取

索引顺序文件如果存放在磁带上，则无法实现随机访问，也就失去了索引的意义，I 错误

Ⅱ显然正确

磁盘上的文件可以直接访问，也可以顺序访问，但如果顺序访问的话，就比较低效了，Ⅲ 正确

对于 Ⅳ，在顺序文件的最后添加新的记录时，则不必须复制整个文件
...
同时，每个盘块都含有指向文件下一个盘块的指针（物理地址）。这些指针对用户透明
...
每个盘块中指向文件下一个盘块的指针（物理地址）做成一个表 文件分配表 FAT
...
> 索引结构可以随机访问且易于扩展
>
> （现在叫 XX 分配，2009 真题叫做 XX 结构，差不多吧）
>
> （虽然隐式链接的 FAT 确实支持随机访问啊……但是这个 2009 年真题就是这么说，我有什么办法……）

##### 例题：连续结构的随机访问优于索引结构的随机访问
...
> 其实我一直有一个莫名的想法是索引结构一定是最好的……这是不对的
>
> 简单来说，如果对于多级索引的话，要找到一个块要需要读多个索引盘块，但是随机访问直接计算得到盘块号，永远只用访问一次盘块
> 
##### 例题：各类分配的计算

2.一个文件系统中有一个 20 MB 大文件和一个 20 KB 小文件，当分别采用连续、链接、链接索引、二级索引和 LINUX 分配方案时，每块大小为 4096 B，每块地址用 4 B 表示。问：
...
连续分配：每个盘块全部放置文件数据，每个块的物理地址相邻

理论上是不受限制，可大到整个磁盘文件区

隐式链接：每个盘块含有一个指向下一个块的指针（物理地址），其他空间放置文件数据

由于块的地址为 4 字节，所以能表示的最多块数为 2^32 = 4 G，而每个盘块中存放文件大小为 4092 字节。链接分配可管理的最大文件为：4 G × 4092 B = 16368 GB

链接索引：每个索引块含有一个指向下一个块的指针（物理地址），其他空间放置指向文件盘块（该块全部空间用于放置文件数据）的指针（物理地址）

已知地址能表示的最多块数为 2^32，又设最多有 n 个索引块，这 n 个索引块及其所表示的文件块要占据整个地址空间，即 1023n + n = 2^32 => n = 2^22

因此可管理的最大文件为 2^22 × 1023 × 4096 B = 16368 GB

二级索引：只有一个一级索引块，该块全部空间用于放置指向二级索引块的指针（物理地址），而二级索引块全部空间用于放置指向文件块的指针（物理地址）

因此可管理的最大文件为 1024 × 1024 × 4096 B = 4 GB

LINUX 分配方案（LINUX 混合分配）：LINUX 的直接地址指针有 12 个，还有一个一级索引，一个二级索引，一个三级索引

因此可管理的最大文件为 12 × 4096 B + 1024 × 4096 B + 1024 × 1024 × 4096 B + 1024 × 1024 × 1024 × 4096 B = 48 KB + 4 MB + 4 GB + 4 TB

(2)

连续分配：对大小两个文件都只需在文件控制块 FCB 中设两项，一是首块物理块块号，另一是文件总块数，不需专用块来记录文件的物理地址

隐式链接：对大小两个文件都只需在文件控制块 FCB 中设两项，一是首块物理块块号，另一是末块物理块块号；同时在文件的每个物理块中设置存放下一个块号的指针

链接索引：

对于小文件：20 KB = 5 个物理块，所以只需一块专用物理块来作索引块，用来保存文件的各个物理块地址

对于大文件：20 MB = 5 K 个物理块，由于链接索引的每个索引块只能保存 (1K-1) 个文件物理块地址（另有一个表目存放下一个索引块指针），所以它需要 6 块专用物理块来作链接索引块，用于保存文件各个的物理地址
...
对 20 KB 小文件只需在文件控制块 FCB 的 i_addr[15] 中使用前 5 个表目存放文件的物理块号，不需专用物理块

对 20MB 大文件，FCB 的 i_addr[15] 中使用前 12 个表目存放大文件前 12 块物理块块号（48K），用一级索引块一块保存大文件接着的 1K 块块号（4M），剩下还有不到 16M，还要用二级索引存大文件以后的块号，二级索引使用第一级索引 1 块，第二级索引 4 块（因为 4 KB × 1 K × 4 = 16 M）。总共也需要 6 块专用物理块来存放文件物理地址
...
都需先计算信息在文件中相对块数，前面信息相对逻辑块号为 5.5K/4K = 1（从 0 开始编号），后面信息相对逻辑块号为 (16 M + 5.5 K)/4 K = 4097。再计算物理块号 = 文件首块号 + 相对逻辑块号，最后只需一次 I/O 操作读出该块信息

链接分配：

为读大文件前面 5.5 KB 的信息，只需先读一次文件头块得到信息所在块的块号，再读一次第 1 号逻辑块得到所需信息，共2次

而读大文件 16 MB + 5.5 KB 处的信息，逻辑块号为 (16M + 5.5K)/4092 = 4107，要先把该信息所在块前面块顺序读出，共化费 4107 次盘 I/O操作，才能得到信息所在块的块号，最后化一次 I/O 操作读出该块信息。所以总共需要 4108 次盘 I/O 才能读取 (16MB + 5.5 KB) 处信息

链接索引：

为读大文件前面 5.5 KB 处的信息，只需先读一次第一个索引块得到信息所在块的块号，再读一次第 1 号逻辑块得到所需信息，共 2 次盘 I/O 操作

为读大文件后面 16MB + 5.5 KB 处的信息，(16 MB + 5.5 KB)/(4 KB × 1023) = 4，需要先 5 次 I/O 操作依次读出各索引块，才能得到信息所在块的块号，再一次 I/O 操作读出该块信息。共 6 次盘 I/O 操作。

二级索引：

为读大文件前面和后面信息的操作相同

首先进行一次 I/O 读第一级索引块，然后根据它的相对逻辑块号计算应该读第二级索引的那块，第一级索引块表目号 = 相对逻辑块号/1 K，对文件前面信息 1/1 K = 0，对文件后面信息 4097/1 K = 4，第二次根据第一级索引块的相应表目内容又用一次  I/O 读第二级索引块，得到信息所在块块号，再进行一次 I/O 读出信息所在盘块，这样读取大文件前面或后面处信息都只需要 3 次盘 I/O 操作。

LINUX 混合分配：

为读大文件前面 5.5 KB 处信息，先根据它的相对逻辑块号，在内存文件控制块 FCB 的 i_addr 第二个表目中读取信息所在块块号，而只需一次 I/O 操作即可读出该块信息

为读大文件后在 (16MB + 5.5KB) 信息，先根据它的相对逻辑块号判断要读的信息是在二级索引管理范围内，先根据 i_addr 内容用一次 I/O 操作读出第一级索引块，再计算信息所在块的索引块号在第一级索引块的表目号为（4097-12-1024)/1024＝2，根据第一级索引块第 3 个表目内容用一次 I/O 操作，读出第二级索引块，就可以得到信息所在块块号，最后用一次 I/O 读出信息所在盘块，这样总共需要 3 次 I/O 操作才能读出文件后面的信息
...
1.设文件 F1 的当前引用计数值为 1，先建立 F1 的符号链接（软链接）文件 F2，再建立 F1 的硬链接文件 F3，然后删除 F1。此时，F2 和 F3 的引用计数值分别是（）
...
建立硬链接时，原文件和硬链接文件的引用计数值都为原文件的引用计数值 + 1

删除符号链接文件的原文件时，删除操作对符号链接文件不可见，也就是不影响符号链接文件的引用计数值；当要使用这个符号链接文件时，如果发现原文件不存在，那么直接删除这个符号链接文件

删除硬链接文件的原文件时，所有相关的硬链接文件的引用计数值都 - 1

因此建立 F2 时，F2 的引用计数值复制 F1 的引用计数值 = 1，建立 F3 时，F1 和 F3 的引用计数值都 = 1 + 1 = 2；

删除 F1 时，对 F2 的引用计数值无影响 = 1，对 F3 的引用计数值 = 2 - 1 = 1
...
> 也就是所谓的：CPU 通过 **I/O 指令**来实现对 DMA 的控制
>
> 不要和 DMA 使用中断结束传输弄混了
>
> 通道是对 DMA 的发展，同理，CPU 通过 **I/O 指令**来实现对通道的控制
...
<table>
    <tr>
        ...
        <td align = center>程序查询</td>
        <td align = center>中断驱动</br>（中断 I/O）</br>（程序中断）</td>
        <td align = center>DMA</td>
        <td align = center>通道</td>
    </tr>
    <tr>
        <td align = center>实现</td>
        <td align = center>完全采用软件</td>
        <td align = center>软硬件结合</br>通过程序实现数据传送</br>中断处理需要硬件实现</td>
        <td align = center>完全采用硬件</br>CPU 通过 <b>I/O 指令</b>来实现对 DMA 的控制</td>
        <td align = center>软硬件结合</br>执行通道程序（由通道指令组成）控制数据交换</br>CPU 通过 <b>I/O 指令</b>来实现对通道的控制</td>
    </tr>
    <tr>
        <td align = center>启动方式</td>
        <td align = center>死循环查询</td>
    ...
    <tr>
        <td align = center>CPU 主程序与外设的串并行</td>
        <td align = center>串行</td>
        <td align = center colspan = 3>并行</td>
    </tr>
    <tr>
        <td align = center>CPU 主程序与传送的串并行</td>
        <td align = center colspan = 2>串行</td>
        <td align = center colspan = 2>并行</br>但是有的题会说不是完全并行，因为还是需要 CPU 主程序通过 <b>I/O 指令</b>来实现对 DMA/通道的控制</td>
    </tr>
    <tr>
        <td align = center>请求资源</td>
        <td align = center colspan = 2>CPU 处理时间</td>
        <td align = center colspan = 2>总线使用权</td>
    </tr>
    <tr>
        <td align = center>数据单位</td>
        <td align = center colspan = 2>字（或字节？）</td>
        <td align = center colspan = 2>一批数据</td>
    </tr>
    <tr>
        <td align = center>响应请求时机</td>
        <td align = center colspan = 2>指令周期结束后</td>
        <td align = center colspan = 2>机器周期结束后，总线事务后</td>
    </tr>
    <tr>
        <td align = center>中断时机</td>
        <td align = center colspan = 2>每个数据</td>
    ...
    <tr>
        ...
        <td align = center colspan = 3>CPU</td>
        <td align = center>通道</td>
    </tr>
    <tr>
        <td align = center>每个控制器所能控制的设备数</td>
        <td align = center colspan = 3>一台</td>
        <td align = center>多台</td>
    </tr>
    <tr>
        <td align = center>适用设备</td>
        <td align = center></td>
        <td align = center>具有中断控制器的外部设备</td>
        <td align = center>具有 DMA 接口的设备</td>
        <td align = center></td>
    </tr>
    <tr>
        <td align = center>场景</td>
        <td align = center colspan = 2>CPU 控制，低速设备</td>
        <td align = center colspan = 2>DMA 控制器/通道程序控制，高速设备</td>
    </tr>
    <tr>
        <td align = center>优先级</td>
        <td align = center colspan = 2>非屏蔽中断高于屏蔽中断</td>
        <td align = center colspan = 2>最高</td>
    </tr>
    <tr>
        <td align = center>异常处理</td>
        <td align = center colspan = 2>能处理异常事件</td>
        <td align = center colspan = 2>仅传送数据</td>
    </tr>
</table>

+ 字节多路通道：用于连接大量低速或中速设备
...
因为单处理机，所以同一时刻只能有一个进程占用处理机

这里说处理机与通道并行……也不算错……而且这是真题

##### 例题：也可以说“处理机与通道不完全并行”

2.对于单 CPU 单通道工作过程，下列可以完全并行工作的是（）
...
1. 从输入队列中取得一块冲满输入数据的缓冲区作为提取输入数据的工作缓冲区 sin 
...
改进方法：建立逻辑设备名与物理设备名的映射机制，**用户编程时提供逻辑设备名**
...
> 注意“磁盘缓冲区”是用来提高 IO 的速度
>
> 我会联想到那个磁盘道数缓冲，可以用来重新整理寻道的次序……这两个不是一个东西
...
1.考虑单用户计算机上的下列 IO 操作，图形用户界面下使用鼠标需要使用缓冲技术（）

答案：✓

在鼠标移动时，若有高优先级的操作产生，为了记录鼠标活动的情况，必须使用缓冲技术

可见，慢速设备也不一定不需要缓冲，缓冲的不一定是高速数据，还可能是一个状态

##### 例题：缓冲区有效的条件

2.若 IO 所花费的时间比 CPU 的处理时间短得多，则缓冲区（）
...
> 就，记录这题的意义在于我或许想到了双缓冲中 T < C+M 的情况
>
> 不冲突的

##### 例题：设备驱动程序的功能

3.设备驱动程序是系统提供的一种通道程序，它专门用于在请求 I/O 的进程与设备控制器之间传输信息。下列选项中不是设备驱动程序的功能的是（）
...
3. 了解 I/O 设备的状态，传送有关参数，设置设备的工作方式

4. 发出 I/O 操作命令，启动 I/O 设备，完成 I/O 操作

5. 响应通道发来的中断请求，根据中断类型调用相应的中断处理程序

6. 构造通道程序

设备驱动程序不能独立完成“控制 I/O 设备的 I/O 操作”

##### 例题：双缓冲区的计算

4.某操作系统采用双缓冲区传送磁盘上的数据。设一次从磁盘将数据传送到缓冲区所用的时间为 T1，一次将缓冲区中的数据传送到用户区所用的时间为 T2(T2 很小，可忽略不计)，CPU 处理数据所用的时间为 T3，则读入并处理该数据共重复 n 次该过程，系统所用总时间约为（）
...
如果 T3 > T1，即 CPU 处理数据比数据传送慢，那么磁盘将数据传送到缓冲区，再传送到到用户区，除了第一次需要耗费的 T1 + T2 + T3 时间，剩余数据可以视为 CPU 进行连续处理，总共花费 (n - 1)T3，读入并处理所用的总时间为 T1 + T2 + nT3

如果 T3 < T1，即 CPU 处理数据比数据传送快，此时可以视为 I/O 连续输入 n 次，共花费 nT1，最后一次 I/O 输入完成时，还需要传送到用户区再处理，即 T2 + T3，读入并处理所用的总时间为 nT1 + T2 + T3

故总时间约为 (n - 1) max(T1, T3) + T1 + T2 + T3
...
计算机开机时需要进行一系列初始化的工作，这些初始化工作是通过执行初始化程序（自举程序）（BIOS）完成的
...
|SCAN|只有磁头移动到最外侧磁道的时候才能往内移动</br>移动到最内侧磁道的时候才能往外移动</br>在做题的时候，访问到了寻道队列的最大值或者最小值也可以掉头，有点令人迷惑……|性能较好，平均寻道时间较短</br>不会产生饥饿现象|只有到达最边上的磁道时才能改变磁头移动方向，事实上，处理了最大和最小的磁道的访问请求之后就不需要再往两边移动磁头了</br>对于各个位置磁道的响应频率不平均|
...
1.一个旋转存储设备上的某个磁道共保存 5 个物理块，物理块按顺序依次编号为 1~5，且旋转一周耗时 20 ms，假设系统收到以下关于该磁道的 I/O 请求序列：读块 2、读块 5、读块 1、读块 4、读块 3，则在对 I/O 请求优化排序后，完成所有请求需要的时间比未优化情况下平均少花费（）
...
虽然没有学过怎么优化，但是很显然，按照块号顺序读取是最快的

所以优化就是把请求按照块号自然递增的顺序排列
...
依次读 <u>2</u> 3 4 <u>5</u> <u>1</u> 2 3 <u>4</u> 5 1 2 <u>3</u> 需要 12 * 4 = 48 ms（加下划线表示是请求的块）
...
依次读 1 2 3 4 5，需要 20 ms

共 30 ms

优化比未优化平均少花费 58 - 30 = 28 ms